...
public class Parser {
     ...
     *
     */
    public Parser(int n, int T, int m, String path, String pathValidation, String pathTrue) {
        this.n = n;
        this.T = T;
        this.m = m;
        breakpoints = new TreeSet<>();
        solutions = readSolution(path, breakpoints);
        validationData = readValidationData(pathValidation);
        trueValues = readTrueValues(pathTrue);

    }

    private double[][][] readTrueValues(String path) {
        double[][][] vals = new double[T][n][n];
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                int i = Integer.parseInt(row[1]), j = Integer.parseInt(row[2]);
                for (int t = 0; t < T; t++) {
                    vals[t][i][j] = Double.parseDouble(row[t + 3]);
                    vals[t][j][i] = vals[t][i][j];
                }
    ...
    private List<double[]>[] readValidationData(String path) {
...
//                    System.out.println(counter+" "+i);
                    matrix[counter][i - 2] = Double.parseDouble(row[i]);
        ...
        List<double[]>[] data = new List[T];
        for (int i = 0; i < data.length; i++) {
            data[i] = new ArrayList<>();
        }
        int length = m / T;
        for (int t = 0; t < T; t++) {
            for (int i = t * length; i < (t + 1) * length; i++) {
...
//        System.out.println(n+" "+T+" "+m);
        Parser parser = new Parser(n, T, m, data, validation, trueData);
...
//        double L0 = 0, L2 = 0, LDiff = 0, norm = 0;
        for (int t = 0; t < trueVals.length; t++) {
            for (int i = 0; i < trueVals[t].length; i++) {
                for (int j = i+1; j < trueVals[t][i].length; j++) {
...
//                System.out.println("");
                ...
                for (int l = 0; l < validationData[t].size(); l++) {
                    ...
                    for (int j = 0; j < precisions[i][t].length; j++) {
                        for (int k = 0; k < precisions[i][t][j].length; k++) {
                            obj += point[j] * point[k] * precisions[i][t][j][k] - Math.log(det);
                        }
                    }
                }
            }
